...
https://www.codewars.com/kata/5263c6999e0f40dee200059d
    ...
    of wrong PINs, they never finally lock the system or sound the alarm. That's why we can 
    ...
    * possible in sense of: the observed PIN itself and all variations considering the adjacent digits
    ...
    PIN with a length of 1 to 8 digits. We could name the function getPINs (get_pins in python, GetPINs in C#). 
    But please note that all PINs, the observed one and also the results, must be strings, because 
...
def get_pins(observed):
    ...
    # To save all the possible numbers
    all_pins = [ keypad[pin] for pin in observed ]
    
    combs = product(*all_pins)
    return [ ''.join(list(x)) for x in combs ]